...
func TestSafrole(t *testing.T) {
	files, err := os.ReadDir(fmt.Sprintf("vectors/safrole/%s", vectorsType))
	require.NoError(t, err, "failed to read directory: vectors/safrole/%s')", vectorsType)
	...
	for _, file := range files {
		...
		t.Run(file.Name(), func(t *testing.T) {
			filePath := fmt.Sprintf("vectors/safrole/%s/%s", vectorsType, file.Name())
			tv, err := ReadSafroleJSONFile(filePath)
...
func ReadSafroleJSONFile(filename string) (*SafroleTestVector, error) {
