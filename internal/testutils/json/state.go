...
func (h Header) To() block.Header {
	...
	if h.EpochMark != nil {
		...
		for i, v := range h.EpochMark.Validators {
			epochMark.Keys[i].Bandersnatch = crypto.BandersnatchPublicKey(hexToHash(v))
		}

		header.EpochMarker = epochMark
	}
...
func (g Guarantee) To() block.Guarantee {
	var guarantee block.Guarantee

	guarantee.WorkReport = g.Report.To()
	...
	for _, sig := range g.Signatures {
		...
		signature.Signature = crypto.Ed25519Signature(hexToBytes(sig.Signature))
...
func (e Extrinsic) To() block.Extrinsic {
	...
	for _, g := range e.Guarantees {
		guarantees = append(guarantees, g.To())
	}
...
func RestoreBlock(b []byte) block.Block {
	var restoredBlock Block
	err := json.Unmarshal(b, &restoredBlock)
	if err != nil {
		panic(fmt.Sprintf("failed to unmarshal block: %v", err))
	}
	return restoredBlock.To()
}
