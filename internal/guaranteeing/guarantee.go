...
func ValidateGuaranteExtrinsicAndReturnReporters(ge block.GuaranteesExtrinsic, s *state.State, validatorState validator.ValidatorState, newEntropyPool state.EntropyPool, chain *store.Chain, newTimeslot jamtime.Timeslot,
	...
	reporters, err := verifySignatures(ge, validatorState, s.PastJudgements.OffendingValidators, newEntropyPool, newTimeslot)
	if err != nil {
		return nil, err
	}
